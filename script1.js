...
function renderTugas() {
    ...
    const tugasTersaring = daftarTugas.filter(function (tugas) {

        if (filterAktif === "selesai" && !tugas.selesai) return false;
        if (filterAktif === "belum" && tugas.selesai) return false;

        if (kataKunciAktif && !tugas.nama.toLowerCase().includes(kataKunciAktif)) return false;
        return true;
    });
    ...
    tugasTersaring.forEach(function (tugas) {
        ...
        nama.addEventListener("click", function () {
            if (waktuKlik !== null) return;
            waktuKlik = setTimeout(function () {
                tugas.coret = !tugas.coret;
                nama.classList.toggle("coret");
                simpanKeStorage();
                waktuKlik = null;
            }, 220);
        });
        ...
        nama.addEventListener("dblclick", function () {
            ...
            function simpanEdit() {
                if (inputEdit.value.trim() === "") {
                    ...
                    inputEdit.replaceWith(nama);
                    return;
                }
                tugas.nama = inputEdit.value.trim();
                simpanKeStorage();
                renderTugas();
            }
        ...
        tombolSelesaiItem.addEventListener("click", function () {
...
function tambahTugas(nama) {
    daftarTugas.push({ id: nextId++, nama: nama, selesai: false, coret: false });
    simpanKeStorage();
    renderTugas();
}

function hapusTugas(id) {
    daftarTugas = daftarTugas.filter(function (tugas) {
        return tugas.id !== id;
    });
    simpanKeStorage();
    renderTugas();
...
function toggleSelesai(id) {
    daftarTugas = daftarTugas.map(function (tugas) {
        if (tugas.id === id) {
...
function muatDariStorage() {
    ...
    if (data) {
        ...
        nextId = daftarTugas.reduce(function (maks, tugas) {
            return Math.max(maks, tugas.id);
        }, 0) + 1;
    }
}
    ...
    daftarCatatan.forEach(function (catatan) {
        ...
        tombolEdit.addEventListener("click", function () {
            ...
            function simpanEdit() {
                ...
                catatan.isi = inputEdit.value.trim();
                simpanCatatanKeStorage();
                renderCatatan();
            }
            ...
            tombolSimpan.addEventListener("click", simpanEdit);
...
function hapusCatatan(id) {
    daftarCatatan = daftarCatatan.filter(function (catatan) {
...
const kutipanCadangan = [
    { content: "Hidup adalah perjalanan, bukan perlombaan. Nikmati setiap langkahnya.", author: "Ralph Waldo Emerson"},
    { content: "Having strong convictions means that you standards or values aren't easily shaken.", author: "karina aespa"},
    { content: "Being nervous is also one of the precious emotions that humans can experience.", author: "Karina aespa"},
    { content: "Akankah semua berjalan semestinya dan semesta berkata semua bisa nyata.", author: "Alkateri"},
    { content: "Hidup mah nggak harus selalu jadi nomor satu, jadi nomor tiga juga gapapa yang penting udah masuk podium.", author: "Adam Adenan, Perunggu"}
];
...
async function ambilKutipan() {
    ...
    try {
        ...
        kutipanElement.textContent = `"${data.content}" — ${data.author}`;
    } catch (error) {
        const acak = kutipanCadangan[Math.floor(Math.random() * kutipanCadangan.length)];
        kutipanElement.textContent = `"${acak.content}" — ${acak.author}`;
    }
}
...
tombolCuaca.addEventListener("click", function () {
    const kota = inputKota.value;
    if (kota.trim() === "") {
        alert("Masukkan nama kota!");
        return;
    }
    ambilCuaca(kota.trim());
});
...
async function ambilCuaca(kota) {
    ...
    try {
        ...
        cuacaSuhu.textContent = `${data.name}: ${Math.round(data.main.temp)}°C`;
        cuacaDeskripsi.textContent = data.weather[0].description;

        statTerasa.textContent = `Terasa ${Math.round(data.main.feels_like)}°C`;
        statKelembapan.textContent = `Kelembapan ${data.main.humidity}%`;
        statAngin.textContent = `Angin ${data.wind.speed} m/s`;

        const sekarang = new Date();
        cuacaWaktu.textContent = "Terakhir diperbarui: " + sekarang.toLocaleTimeString("id-ID");
    } catch (error) {
        cuacaSuhu.textContent = error.message;
        cuacaDeskripsi.textContent = "";
    }
}
...
async function muatSemuaWidget() {
    if (statusElement) statusElement.textContent = "Memuat data...";
    await Promise.all([ambilKutipan(), ambilCuaca(inputKota.value)]);
    if (statusElement) statusElement.textContent = "Data berhasil dimuat";
}
...
function aktifkanDragDrop() {
    ...
    items.forEach(function (item) {
        ...
        item.addEventListener("drop", function (event) {
            ...
            const indexAsal = daftarTugas.findIndex(function (t) { return t.id === idAsal; });
            const indexTujuan = daftarTugas.findIndex(function (t) { return t.id === idTujuan; });
...
const cariTugasDebounced = debounce(function (kataKunci) {
    kataKunciAktif = kataKunci.toLowerCase();
    renderTugas();
}, 300);

cariTugas.addEventListener("input", function (event) {
